...
| `MCP_UPSTREAM_BEARER_FILE` | no | Path to a file holding a credential for the upstream, sent as `Authorization: Bearer <contents>` on every admitted request, after the caller's own `Authorization` is stripped. For an upstream that wants a static token of its own. Read once at startup; a set but unreadable, empty or multi-token file stops startup. Unset: the upstream receives no `Authorization`. |
